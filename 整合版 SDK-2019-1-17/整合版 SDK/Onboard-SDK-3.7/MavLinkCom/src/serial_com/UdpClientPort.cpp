...
public:
	...
	int read(uint8_t* result, int bytesToRead)
	{
		...
		// std::cout<<"udp read buffer"<<std::endl;
		while (!closed_)
		{
			// std::cout<<"udp read buffer run..."<<std::endl;
			socklen_t addrlen = sizeof(sockaddr_in);
			// std::cout<<"udp read buffer run 1"<<std::endl;
			#if defined (__APPLE__)
			int a = 1;
			#elif defined _WIN32
			int b = 1;
			...
			// std::cout<<"udp read buffer run 2"<<std::endl;
			#endif
			if (rc < 0)
			{
				// std::cout<<"udp read buffer run 3"<<std::endl;
				int hr = WSAGetLastError();
				// std::cout<<"udp read buffer run 4"<<std::endl;
#ifdef _WIN32
				if (hr == WSAEMSGSIZE)
				{
				...
				// std::cout<<"udp read buffer run 5"<<std::endl;
			}
			// std::cout<<"udp read buffer run 6"<<std::endl;
			if (remoteaddr.sin_port == 0)
			{
				// we now have it.
			...
			// std::cout<<"udp read buffer run 7"<<std::endl;
			if (rc == 0)
			{
				// std::cout<<"udp read buffer run 8-1"<<std::endl;
				//printf("Connection closed\n");
				return -1;
			}
			else
			{
				// std::cout<<"udp read buffer run 8-2"<<std::endl;
				return rc;
			}
		}
		return -1;
